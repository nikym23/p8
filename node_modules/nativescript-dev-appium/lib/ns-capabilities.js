...
class NsCapabilities {
    constructor() {
        ...
        this._appiumCaps = capabilities_helper_1.resolveCapabilities(this._appiumCapsLocation, parser.runType, parser.projectDir);
    ...
    set appPath(appPath) { this._appPath = appPath; }
    get device() { return this._device; }
    set device(device) { this._device = device; }
    get emulatorOptions() { return (this._emulatorOptions || "-wipe-data -gpu on"); }
    isAndroidPlatform() { return this._appiumCaps.platformName.toLowerCase().includes("android"); }
    resolveAppPath() {
        if (this._appPath) {
            this._appiumCaps.app = this._appPath;
        }
        if (!this._appiumCaps.app) {
            this._appiumCaps.app = utils_1.getAppPath(this);
        }
        console.log("Application full path: " + this._appiumCaps.app);
    }
    checkMandatoryCapabiliies() {
        if (!this.isSauceLab && !utils_1.fileExists(this._appiumCaps.app)) {
